...
public class FieldOfView : MonoBehaviour
{
    ...
    public void updateValues(float radius, float angle, float FOVcoef)
    {
        this.maxViewAngle = angle;
        this.FOVcoef = FOVcoef;
    ...
    void LateUpdate()
    {
        // TODO: mudar o FiledOfView
        
        if (aiming)
        {
            float result = viewAngle -= FOVcoef * Time.deltaTime;
            if (result < 1f)
                viewAngle = 1f;
            else 
                viewAngle -= FOVcoef * Time.deltaTime;
        }
    ...
    void FindVisibleTargets()
    {
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            Vector3 dirToTarget = (target.position - viewMeshFilter.transform.position).normalized;
            if (Vector3.Angle(viewMeshFilter.transform.forward, dirToTarget) < viewAngle / 2)
            {
                float dstToTarget = Vector3.Distance(viewMeshFilter.transform.position, target.position);
                if (!Physics.Raycast(viewMeshFilter.transform.position, dirToTarget, dstToTarget, obstacleMask))
                {
